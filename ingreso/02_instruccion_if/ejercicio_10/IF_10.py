...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        if aleatorio >=1 and aleatorio <=3:
            mensaje = f"desaprobado la nota es {aleatorio}" 
        elif aleatorio >=4 and aleatorio <=5:
            mensaje = f"aprobado la nota es {aleatorio}"
        else:
            mensaje = f"promocion directa la nota es {aleatorio}"
